...
def run_unified_work(
    ...
    if input_type == 'todo':
        ...
        console.print("[yellow]Unknown input type. Please provide a todo ID, plan file, or pattern.[/yellow]")
...
def _run_react_todo(pattern: str, dry_run: bool, parallel: bool = True, max_workers: int = 3, in_place: bool = True):
        ...
        try:
            ...
            try:
                ...
                if result.success_status:
                    ...
                    return {"todo_id": todo['id'], "success": True, "summary": result.resolution_summary}
                else:
                    console.print(f"[red]Failed:[/red] {result.resolution_summary}")
                    return {"todo_id": todo['id'], "success": False, "summary": result.resolution_summary}
            finally:
                if worktree_path:
                    os.chdir(original_dir)
    ...
    try:
        if parallel and len(todos) > 1 and not dry_run:
            console.print(f"[dim]Executing {len(todos)} todos in parallel with {max_workers} workers[/dim]")
            with ThreadPoolExecutor(max_workers=min(max_workers, len(todos))) as executor:
                futures = {executor.submit(resolve_todo_task, todo): todo for todo in todos}
                results = []
...
def _run_react_todo_batch(pattern: str, dry_run: bool, parallel: bool = True, max_workers: int = 3, in_place: bool = True):
